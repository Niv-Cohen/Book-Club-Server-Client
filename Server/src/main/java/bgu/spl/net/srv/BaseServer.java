        ...
        try (ServerSocket serverSock = new ServerSocket(port)) {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                BlockingConnectionHandler<T> handler = new BlockingConnectionHandler<>(
                        ...
                        encdecFactory.get(),
                        protocolFactory.get());
                        connections.addTo_id_HandlerMap(id.get(),handler);
                        handler.getProtocol().start(id.get(), connections);
                        //add to handlers list
                        //create List for certain client in userId_SubId_NameMap
                        connections.addNewHandler(id.getAndIncrement(),handler);
                execute(handler);
            }
